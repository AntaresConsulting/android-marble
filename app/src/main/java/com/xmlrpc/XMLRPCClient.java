...
// reader = new StringReader("<?xml version='1.0'?><methodResponse><params><param><value>\n\n\n</value></param></params></methodResponse>");
			...
			if (tag.equals(Tag.FAULT)) {
				...
				entity.consumeContent();
				throw new XMLRPCFault(faultString, Integer.parseInt(faultCode));
			} else {
				entity.consumeContent();
	...
	public Object call(String method, Object p0) throws XMLRPCException {
		...
		if(p0 instanceof List){
			paramsRes = new Object[((List)p0).size()];
			int i = 0;
			for (Object object : (List)p0) {
				paramsRes[i++]=object;
			}
		}else{
			paramsRes= new Object[]{p0};
		}
		Object test =  callEx(method, paramsRes);
		return test;
	}
